...
class ChatController {
    ...
    async fetchLoop(sessionId) {
        ...
        while (this.activeSessionId === sessionId) {
            try {
                if (!this.videoElement || !this.videoStartTime) {
                    await new Promise(r => setTimeout(r, 2000));
                    continue;
                }

                // Poll the chat state at the current video playhead
                const vidT = this.videoStartTime.getTime() + (this.videoElement.currentTime * 1000);
                const targetTime = new Date(vidT);
                
                // Kick API start_time returns the ~75 messages immediately preceding the timestamp
                let url = `https://kick.com/api/v2/channels/${this.channelId}/messages?start_time=${targetTime.toISOString()}`;
                
                const res = await fetch(url);
                if (!res.ok) { 
                    console.warn(`Kick Unlocker: API returned ${res.status}. Retrying...`);
                ...
                if (msgs.length) {
                    ...
                    if (added) {
                        ...
                        if (this.messages.length > 500) {
                            this.messages = this.messages.slice(this.messages.length - 500);
                        }
                        if (this.videoElement) this.updateUI(this.videoElement.currentTime);
                    }
                }

                // Poll every 5 seconds (fetches up to 75 msgs, which supports chat speed up to 15 msgs/sec)
                await new Promise(r => setTimeout(r, 5000));
            } catch (e) { 
                console.error("Kick Unlocker Chat Error:", e);
                await new Promise(r => setTimeout(r, 10000)); 
            }
        }
    }
    ...
    updateUI(cwdSeconds) {
        ...
        if (!lastM || this.lastRenderedMsgId === lastM.id) return;

        this.chatList.innerHTML = subset.map(msg => `
            <div style="margin-bottom:4px;line-height:1.4;word-wrap:break-word;">
                <span style="color:${msg.sender?.identity?.color || '#53fc18'};font-weight:bold;margin-right:5px;">${msg.sender?.username || 'System'}:</span>
